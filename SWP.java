...
public class SWP {
   ...
   */
  // Our variables
  boolean no_nak = true; /* global variable to record whether no ack has been sent yet */
  
  // Timers for each frame
  Timer[] f_timer = new Timer[NR_BUFS];
  Timer ack_timer; // Timer only for acknowledgement, to avoid the destruction by loss of acknowledgement
  ...
  // Send three types of frames, data, nak or ACK
  ...
  public void protocol6() {
    ...
    int too_far = NR_BUFS;
    PFrame reveicedFrame = new PFrame();
    ...
    while (true) {
      ...
      case (PEvent.FRAME_ARRIVAL): {
        from_physical_layer(reveicedFrame);

        if (reveicedFrame.kind == PFrame.DATA) {

          if((reveicedFrame.seq != frame_expected) && no_nak) {
            send_frame(PFrame.NAK, 0, frame_expected, out_buf);
          } else {
             start_ack_timer();
          }

          if (between(frame_expected, reveicedFrame.seq, too_far) && (arrived[reveicedFrame.seq % NR_BUFS]==false)){
            
            arrived[reveicedFrame.seq % NR_BUFS] = true;
            in_buf[reveicedFrame.seq  % NR_BUFS] = reveicedFrame.info;
        ...
        if((reveicedFrame.kind == PFrame.NAK) && between(ack_expected, (reveicedFrame.ack + 1)%(MAX_SEQ + 1),next_frame_to_send)) {

          send_frame(PFrame.DATA, (reveicedFrame.ack+1)%(MAX_SEQ+1), frame_expected, out_buf);
        }
        
        // For selective frame, cover all the arrived frames
        while(between(ack_expected, reveicedFrame.ack, next_frame_to_send)){
          //nbuffered--;
      ...
      case (PEvent.CKSUM_ERR): {
        // Data has error , send an nak if no nak sent yet 
        if(no_nak) {
          send_frame(PFrame.NAK, 0, frame_expected, out_buf);
        }
      }
        break;
      case (PEvent.TIMEOUT): {
        // Frame times out, resend the frame
  ...
  private void start_timer(int seq) {
    stop_timer(seq);
        ...
        f_timer[seq % NR_BUFS].schedule(new f_task(seq), 100);
  }
